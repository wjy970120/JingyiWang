...
/** Graphics class Circle allows the drawing of circles.<br>
 * Circles use an (x1, y1, diameter) coordinate system of where they are located<br>
 * on the board.<br>
 * Each Circle is tied to and represents a BoardElement. <br>
 * Circles can be tied to instances of classes Node, Truck, and Parcel. <br>
 * While a Circle can technically be tied to an instance of class Edge, <br>
 * this option is not expected and may result in irregular behavior.
 *
 * @author MPatashnik */
public class Circle extends JPanel {
	...
	protected GraphElement represents;
	...
	/** Constructor: an instance at (x, y) of diameter d and colored black <br>
	 * that represents r and is not filled. */
	public Circle(GraphElement r, int x, int y, int d) {
		this(r, x, y, d, null, false);
	}

	/** Constructor: an instance at (x, y) of diameter d and colored c <br>
	 * that represents r and is filled iff filled. */
	public Circle(GraphElement r, int x, int y, int d, Color c, boolean filled) {
		represents= r;
	...
	/** Set the x coordinate of this circle to x. <br>
	...
	/** Set the color of this circle to c. <br>
	 * Throw an IllegalArgumentException if c is not in Score.COLORS. */
	...
	/** Return the diameter field of this circle to d and <br>
	...
	/** Change the color according to the Color Policy. DOES NOTHING */
	...
	/** Fix the boundaries so that all drawings will be within the bounds. <br>
	...
	public DPoint getVectorTo(Circle c) {
		return new DPoint(c.x1 - x1, c.y1 - y1);
	}
