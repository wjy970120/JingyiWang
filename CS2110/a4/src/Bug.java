...
/** An instance represents a bug spreading and ultimately dying out among<br>
 * a limited population (or killing everyone). <br>
 * <br>
 * Each Bug is created on a Network of people and with a chosen first patient. <br>
 * Bug is runnable, but for the purposes of this project, it does not need to <br>
 * be run on a separate thread.
 *
 * @author MPatashnik, revised by gries. */
public class Bug implements Runnable {
	/** The graph on which this Bug is running. */
	private Network network;

	/** The tree representing this bug spreading. */
	private BugTree tree;

	/** Number of time steps this bug took to create dt. */
	private int steps;

	/** The bug model: Statistics that determine the spread of the disease. */
	...
	/** Constructor: a new Bug on network nw with first patient fp and disease model s. */
	public Bug(Network nw, Person fp, Statistics s) {
		steps= 0;
		network= nw;
		fp.becomeSick(0);
		tree= new BugTree(fp);
	...
	private void step() {
		...
		for (Person p : people) {
			if (p.isSick()) {
				...
				if (n != null && n.isHealthy() && statistics.bugSpreadsToPerson()) {
					n.becomeSick(steps);
					tree.insert(p, n);
				}
			}
	...
	/** Run Bug on the arguments listed in args. <br>
	 * If args doesn't match the pattern below, read in arguments via the console by using
	 * readArgs().
	 *
	 * Then, call disease.run() and create a BugFrame showing the created BugTree.
	 *
