...
public class Heap<E> {
	...
	private int goodChild(int k) {
		if (k >= size || 2 * k + 1 >= size)
			return k;
		else if (2 * k + 1 == size - 1)
			return 2 * k + 1;
		else if (compareTo(2 * k + 1, 2 * k + 2) > 0)
			return 2 * k + 1;
		else return 2 * k + 2;
	}
	...
	public void changePriority(E v, double p) {
		...
		if (!map.containsKey(v)) throw new IllegalArgumentException("v is not in the heap.");
		int h= map.get(v);
		double o= b[h].priority;
		b[h].priority= p;
		if (compareTo(p, o) > 0) bubbleUp(h);
		else if (compareTo(p, o) < 0) bubbleDown(h);
