...
/** An instance is a JFrame with a bug-spreading tree and a panel to display information
 *
 * @author Mshnik */
public class BugFrame extends JFrame {
	...
	/** Return a new BugFrame to display dt and s, the number of time steps to create it. */
	public static BugFrame show(BugTree dt, int s) {
		return new BugFrame(dt, s);
	}
	...
	/** BugTree being displayed. */
	private BugTree root;
	...
	/** GraphPanel that shows the BugTree. */
	...
	/** Display the bug route from the previously selected person to the selected person. */
	private JLabel twoPersonBugRouteLabel= new JLabel();
	...
	/** The bug route from the previously selected person to the currently selected person. */
	private List<Person> bugRoutePrevToSelected;

	/** Constructor: a visible GUI for dt, with s time steps needed to construct it. */
	private BugFrame(BugTree dt, int s) {
		super();
		setTitle("A4 BugTree Explorer");
	...
	private void createAndAddCircleMouseListener() {
		graphPanel.addCircleMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					bugRoutePrevToSelected= root.getTree(previouslySelectedPerson)
						.bugRouteTo(selectedPerson);
	...
	private void fixInformationPanel(JPanel infoPanel) {
		...
		infoPanel.add(twoPersonBugRouteLabel);
		infoPanel.add(twoPersonSharedAncestorLabel);
	}

	/** Lay out the nodes in the BugTree. Specifically, call recLayoutNodes(...) [below], with
	...
	private void recLayoutNodes(BugTree dt, float xMin, float xMax, float yPercent,
		float yInc) {
	...
	/** Return a stroke (for drawing) for line l. If the line is part of the bugRoute, <br>
	...
	private Stroke getStrokeFor(GraphPanel<Person, PersonConnection>.Line l) {
		if (bugRoutePrevToSelected != null) {
			...
			if (bugRoutePrevToSelected.contains(p1) &&
				bugRoutePrevToSelected.contains(p2)) {
	...
	private void populateLabels() {
		...
		populateLabel(() -> root.widthAtDepth(root.depthOf(p)), selectedWidthLabel,
		...
		populateLabel(() -> root.getTree(p).numberOfChildren(), selectedPersonChildrenCountLabel,
			"Children");
		populateLabel(() -> root.getTree(p).size(), selectedPersonSubtreeCountLabel,
			"Subtree Size");
		populateLabel(() -> root.getTree(p).maxDepth(), selectedPersonMaxDepthLabel,
			"Subtree depth");
		populateLabel(() -> root.getTree(p).maxWidth(), selectedPersonMaxWidthLabel,
			"Subtree width");
		...
		if (pp != null) {
			populateLabel(() -> {
				List<Person> route= root.getTree(pp).bugRouteTo(p);
				if (route == null) return null;
				else return Arrays.deepToString(route.stream().map(a -> a.getName()).toArray());
			}, twoPersonBugRouteLabel, "Bug Route");
			populateLabel(() -> {
				Person parent= root.sharedAncestorOf(pp, p);
